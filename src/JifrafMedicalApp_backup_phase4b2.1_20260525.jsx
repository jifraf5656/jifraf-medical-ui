...
export default function JifrafFuturisticApp() {
  ...
  const generateProtocolId = () => {
    ...
    return `JFR-${yyyy}${mm}${dd}-${hh}${min}${ss}`;
  };
  ...
  useEffect(() => {
    // 1. Protokol ID'yi Oluştur ve Set Et
    setProtocolId(generateProtocolId());
    ...
    fetch('http://127.0.0.1:9000/api/advisory/run', {
      ...
      .then((data) => {
        if (data.draft_report) {
          ...
          setShowEpikrizModal(true); // Automatically launch the print-friendly White Clinical Paper modal
        } else {
  ...
  const handleCreatePatient = () => {
    ...
    if (intakeMode === 'emergency') {
      ref = `Acil Geçici Hasta (${protocolId})`;
      ...
      ref = `${patientFirstName.trim()} ${patientLastName.trim()} (${protocolId})`;
    ...
    <div className={`fixed inset-0 bg-[#010308] flex flex-col p-4 font-sans text-slate-300 transition-colors duration-300 ${language === 'ar' ? 'rtl' : 'ltr'}`}>
      ...
      <div className="w-full flex-1 max-w-[1920px] mx-auto bg-[#020617] flex flex-col p-4 gap-3 rounded-2xl border-2 border-cyan-500 shadow-[0_0_30px_rgba(6,182,212,0.2)] overflow-hidden relative selection:bg-cyan-900">
        ...
        <header className="flex flex-wrap items-center gap-3 shrink-0">
          ...
          {registeredPatient && (
            <div className="flex items-center gap-3 px-3.5 py-1.5 border border-cyan-500/40 bg-cyan-950/20 rounded-full shadow-[0_0_10px_rgba(6,182,212,0.15)] shrink-0 max-w-xs transition-all hover:border-cyan-400 select-none group">
              <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></div>
              <div className="leading-tight text-[11px] font-sans">
                <span className="text-slate-100 font-extrabold block truncate tracking-wide">
                  {intakeMode === 'emergency' ? "Acil Geçici Hasta" : `${patientFirstName} ${patientLastName}`}
                </span>
                <span className="text-[9px] text-slate-400 block font-mono">
                  DOB: {intakeMode === 'emergency' ? `${intakeBirthYear} (Beyan)` : patientBirthDate} | {new Date(registeredPatient.created_at).toLocaleDateString()}
                </span>
              </div>
            </div>
          )}
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3 min-h-0">
              ...
              <div className="flex-1 border border-cyan-500/60 rounded-xl bg-slate-900/30 flex flex-col min-h-0 shadow-[0_0_15px_rgba(6,182,212,0.15)] overflow-y-auto [&::-webkit-scrollbar]:hidden p-4">
                <h2 className="text-cyan-400 font-semibold flex items-center justify-between border-b border-cyan-500/30 pb-3 mb-4 shrink-0 text-sm md:text-base">
                  <div className="flex items-center gap-2">
                    ...
                    <span>Kabul Formu / Clinical Workstation Ingestion</span>
                  </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 leading-normal font-sans text-slate-300">
          ...
          <div className="lg:col-span-3 border border-orange-500/40 rounded-xl bg-slate-900/40 p-4 flex flex-col gap-4 overflow-y-auto [&::-webkit-scrollbar]:hidden shadow-[0_0_15px_rgba(249,115,22,0.1)]">
            ...
            {advisoryResult && (
              <button
                type="button"
                onClick={() => setShowEpikrizModal(true)}
      ...
      {/* 4. EPİKRİZ WHITE CLINICAL PAPER MODAL */}
      {showEpikrizModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 backdrop-blur-sm overflow-y-auto no-print p-4">
          <div 
            id="epikriz-modal-content"
            className="w-full max-w-4xl bg-white text-slate-800 shadow-2xl rounded-xl border border-slate-350 overflow-hidden flex flex-col my-8 select-text font-serif leading-relaxed shadow-[0_20px_50px_rgba(0,0,0,0.3)] animate-in fade-in zoom-in-95 duration-200"
          >
            ...
            {/* Modal Header (Cyan bar in app, but clean in print) */}
            <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center shrink-0 no-print">
              <div className="flex items-center gap-2">
                <FileText className="w-5 h-5 text-slate-600" />
                <span className="font-sans font-bold text-slate-700 text-sm">Resmi Klinik Epikriz Belgesi</span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => window.print()}
                  className="px-3.5 py-1.5 bg-cyan-600 text-white rounded-lg text-xs font-bold font-sans hover:bg-cyan-700 shadow-md cursor-pointer transition-colors"
                >
                  Yazdır / PDF Kaydet (Ctrl+P)
                </button>
                <button
                  type="button"
                  onClick={() => setShowEpikrizModal(false)}
                  className="px-3 py-1.5 bg-slate-200 text-slate-700 rounded-lg text-xs font-bold font-sans hover:bg-slate-300 cursor-pointer transition-colors"
                >
                  Kapat
                </button>
              </div>
            ...
            {/* White Paper Content */}
            <div className="flex-1 bg-white p-8 md:p-12 overflow-y-auto max-h-[80vh] print:max-h-none print:overflow-visible">
              ...
              <div className="text-center border-b-2 border-slate-800 pb-6 mb-8 relative">
                ...
                <div className="w-12 h-12 mx-auto mb-2 border border-slate-350 rounded-full flex items-center justify-center bg-slate-50">
                  <Activity className="w-6 h-6 text-slate-850" />
                </div>
              ...
              {/* 1. Hasta Bilgileri / Patient Info */}
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-3 gap-x-6 text-xs text-slate-800 font-sans">
                  ...
                  <div>
                    ...
                    <span className="font-mono font-bold text-slate-900">{protocolId}</span>
                  </div>
                  <div>
              ...
              {/* 2. Başvuru Nedeni / Admitting Complaint */}
              <div className="mb-6">
                ...
                <p className="text-xs text-slate-800 leading-relaxed pl-2 border-l-2 border-slate-400">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  ...
                  <div className="border border-slate-200 p-3 rounded space-y-2">
                    ...
                    <ul className="list-disc pl-4 text-slate-700 font-sans space-y-1 text-xs">
              ...
              <div className="mb-6">
                ...
                {advisoryData?.correlation_signals && advisoryData.correlation_signals.length > 0 ? (
                  <div className="space-y-2 pl-2 border-l-2 border-slate-400">
                    {advisoryData.correlation_signals.slice(0, 5).map((signal, idx) => (
                      <div key={idx} className="text-xs">
                        • <strong>{signal.rule_name}</strong> - Gerekçe: <span className="text-slate-700">{signal.explanation}</span> <span className="font-mono text-[10px] bg-slate-105 px-1 border rounded text-slate-500 ml-1.5">Grade: {signal.evidence_grade}</span>
                      </div>
                    ))}
                  </div>
                ) : (
              ...
              <div className="mb-6">
                <h3 className="text-xs font-bold text-slate-900 uppercase font-sans tracking-widest border-b border-slate-300 pb-1.5 mb-2">7. ACİL MÜDAHALE VE UYGULANAN İŞLEMLER</h3>
                <div className="no-print mb-2">
                  <textarea 
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded p-2 text-xs font-sans text-slate-800 focus:outline-none focus:border-cyan-500"
                  />
                </div>
              ...
              <div className="mb-6">
                <h3 className="text-xs font-bold text-slate-900 uppercase font-sans tracking-widest border-b border-slate-300 pb-1.5 mb-2">8. UYGULANAN TEDAVİLER</h3>
                <div className="no-print mb-2">
                  <textarea 
                    rows={2}
              ...
              <div className="mb-6">
                <h3 className="text-xs font-bold text-slate-900 uppercase font-sans tracking-widest border-b border-slate-300 pb-1.5 mb-2">9. TABURCULUK REÇETESİ</h3>
                <div className="no-print mb-2">
                  <textarea 
                    rows={2}
              ...
              <div className="mb-6">
                <h3 className="text-xs font-bold text-slate-900 uppercase font-sans tracking-widest border-b border-slate-300 pb-1.5 mb-2">10. CLINICIAN VERIFIED FINAL NOTE</h3>
                <div className="no-print mb-2">
                  <textarea 
                    rows={2}
                    value={clinicianNote}
              ...
              {/* 11. İmza / Mühür Alanı */}
              <div className="mt-12 pt-8 border-t border-slate-300 flex justify-between items-start text-xs font-sans text-slate-850">
                <div className="text-left">
                  <p className="font-bold">Düzenleyen Kurum:</p>
