...
export default function JifrafFuturisticApp() {
  ...
  const handleSidebarClick = (tabName) => {
    ...
    const dockableTabs = ['intake', 'anamnez', 'radyoloji', 'ekg', 'steteskop', 'ozet', 'uyarilar', 'epikriz'];
  ...
  return (
    <div className={`fixed inset-0 bg-[#010308] flex flex-col p-4 font-sans text-slate-300 transition-colors duration-300 ${language === 'ar' ? 'rtl' : 'ltr'}`}>
      ...
      <div className="w-full flex-1 max-w-[1920px] mx-auto bg-[#020617] flex flex-col p-4 gap-3 rounded-2xl border-2 border-cyan-500 shadow-[0_0_30px_rgba(6,182,212,0.2)] overflow-hidden relative selection:bg-cyan-900">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3 min-h-0">
          ...
          <div className="hidden lg:flex lg:col-span-2 flex-col border border-cyan-500/50 rounded-xl bg-slate-900/40 p-2 gap-1 overflow-y-auto [&::-webkit-scrollbar]:hidden shadow-[0_0_15px_rgba(6,182,212,0.1)]">
            ...
            <SidebarItem icon={Crosshair} label={t("indic")} active={activeTab==='endikasyon'} onClick={()=>handleSidebarClick('endikasyon')} />
      ...
      {openPanels.anamnez && (
        <WorkspaceWindow
          id="anamnez"
          title="Anamnez & EKG Canlı Sinyal"
          icon={ClipboardList}
        ...
        >
          <div className="flex flex-col min-h-0">
            <div className="flex justify-between items-center p-3 border-b border-cyan-500/30 bg-cyan-950/20 shrink-0">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  onClick={() => pdfInputRef.current?.click()} 
                  disabled={loading || !caseId || isSignedOff}
                  className="text-[10px] flex items-center gap-1 border border-slate-600 px-2 py-1 rounded hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer font-sans"
                >
                  <Upload className="w-3 h-3"/> {loading ? 'Yükleniyor...' : 'PDF Yükle'}
                </button>
                <button 
            ...
            <div className="flex-1 flex flex-col lg:flex-row p-3 gap-4 overflow-y-auto [&::-webkit-scrollbar]:hidden">
              <div className="w-full lg:w-1/3 space-y-3 font-sans">
                {displayAnamnesis.map((item, i) => (
                  <div key={i}>
                    <label className="text-[10px] text-slate-400 block mb-1">{item.label}</label>
                    <div className="border border-cyan-800 bg-cyan-950/30 p-2 rounded text-xs text-cyan-200">{item.value}</div>
              ...
              <div className="w-full lg:w-2/3 border border-cyan-800/50 rounded-lg bg-[#020814] relative flex flex-col overflow-hidden min-h-[250px]">
                <div className="absolute inset-0 bg-[linear-gradient(rgba(6,182,212,0.15)_1px,transparent_1px),linear-gradient(90deg,rgba(6,182,212,0.15)_1px,transparent_1px)] bg-[size:20px_20px]"></div>
                ...
                <div className="absolute top-1/3 right-1/4 bg-red-950/80 border border-red-500 text-red-400 text-[10px] px-2 py-1 rounded-full z-10 flex items-center gap-1 font-sans">
                  <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-ping"></span> ST Elevation (V1-V3, red)
                </div>
                
                <div className="flex-1 flex flex-col justify-center p-4">
                  <MockEKGLine label="DI" highlight={false} />
                  <MockEKGLine label="DII" highlight={false} />
                  <MockEKGLine label="V1" highlight={true} />
                  <MockEKGLine label="V2" highlight={true} />
                  <MockEKGLine label="V3" highlight={true} />
      ...
      {openPanels.radyoloji && (
        ...
        >
          <UploadPanel 
            title="Radyoloji Gözlem Giriş Modülü (Imaging/DICOM)"
            icon={FileSearch}
            themeColor="cyan"
            accept=".jpg,.jpeg,.png,.webp,.pdf,.dcm,.dicom"
            acceptLabel="DICOM, PDF, PNG, JPEG, WEBP"
      ...
      {openPanels.ekg && (
        ...
        >
          <UploadPanel 
            title="Elektrokardiyografi (EKG) Sinyal Giriş Modülü"
            icon={Activity}
            themeColor="emerald"
            accept=".jpg,.jpeg,.png,.webp,.pdf"
            acceptLabel="PDF, PNG, JPEG, WEBP"
      ...
      {openPanels.ozet && (
        ...
        >
          <div className="flex flex-col min-h-[500px] xl:h-full relative overflow-hidden">
            {/* Scanline overlay */}
            <div className="hud-scanline"></div>
            ...
            <div className="flex-1 grid grid-cols-1 xl:grid-cols-12 p-4 gap-4 overflow-y-auto [&::-webkit-scrollbar]:hidden z-10">
              ...
              {/* RIGHT COLUMN: Live Terminal & Hekim Advisory Report (Col 8-12) */}
              <div className="xl:col-span-5 flex flex-col gap-3 min-h-0 overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-slate-800 [&::-webkit-scrollbar-thumb]:rounded-full font-sans">
                <div className="flex justify-between items-center pb-1 border-b border-slate-800 shrink-0">
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                    {isSignedOff ? "Clinician Verified Report & Sign-off" : t("hekim_vaka_ozeti")}
                  </h3>
...
function WorkspaceWindow({ id, title, icon: Icon, isOpen, isMinimized, isActive, isFullscreen, onClose, onMinimize, onFullscreen, onFocus, children }) {
  ...
  return (
    <div 
      onClick={onFocus}
      className={`absolute transition-all duration-200 flex flex-col border rounded-2xl shadow-2xl overflow-hidden ${
    ...
    >
      ...
      <div className={`flex items-center justify-between px-4 py-2 border-b shrink-0 select-none cursor-pointer ${
        isEpicrisis 
